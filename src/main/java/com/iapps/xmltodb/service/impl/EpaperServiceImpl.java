...
public class EpaperServiceImpl implements EpaperService {
	...
	public EpaperDto uploadXmlToDB(MultipartFile file) throws Exception {
		...
		if (file.getContentType().contains("text/xml") || file.getContentType().contains("application/xml")) {
			...
			epaper = Epaper.builder().newsPaperName(epaperRequest.getDeviceInfo().getName())
	...
	@Override
	public Page<EpaperDto> getEpaperList(Integer pageNo, Integer pageSize, String sortBy, Boolean asc, String search)
			throws Exception {
		...
		if (Strings.isNotBlank(search)) {
			return epaperRepository.findByNewsPaperNameLikeIgnoreCase(pageable, "%" + search + "%")
					.map(EpaperConverter::convert);
		} else
